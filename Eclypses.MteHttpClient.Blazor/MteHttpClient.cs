...
using Eclypses.MteHttpClient.Blazor.Models;
    ...
    public class MteHttpClient : IMteHttpClient
    {
        ...
        public async Task<HttpResponseMessage> MteGetAsync(string route, Dictionary<string, string>? headers = null, string? relayIdentifier = "", int getAttempts = 0)
        {
            ...
            try
            {
                ...
                //
                // endpoint.mteRelayClient.DefaultRequestHeaders.Remove(Constants.MTE_RELAY_HEADER);
                ...
                headers.Add(Constants.MTE_RELAY_HEADER, MakeMteRelayHeaderValue(endpoint, true));
                //
...
#pragma warning restore CS1998 // Async method lacks 'await' operators and will run synchronously
        ...
        public async Task<HttpResponseMessage> MtePostAsync(string route, HttpContent content, Dictionary<string, string>? headers = null, string? relayIdentifier = "", int postAttempts = 0)
        {
            ...
            try
            {
                ...
                headers.Remove(Constants.MTE_RELAY_HEADER);
                endpoint.XmteRelayHeader.bodyIsEncoded = true;
                endpoint.XmteRelayHeader.pairId = GetNextPairId(endpoint);
                headers.Add(Constants.MTE_RELAY_HEADER, endpoint.XmteRelayHeader.ToString());
                //
...
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
        ...
        private MteRelayEndpoint? GetEndpointFromId(string relayIdentifier)
        {
            ...
            return _mteRelayOptions.Endpoints.Where(e => e.HttpClientRelayName!.Equals(relayIdentifier, StringComparison.OrdinalIgnoreCase)).FirstOrDefault();
        }
        #endregion
...
#pragma warning restore CS8601 // Possible null reference argument.
        ...
        private async Task<bool> PairWithTheMTERelay(MteRelayEndpoint endpoint)
        {
            ...
            {                
                ...
                // Ensure that the required header is present.
                //
                endpoint.mteRelayClient!.DefaultRequestHeaders.Add(Constants.MTE_RELAY_HEADER, MakeMteRelayHeaderValue(endpoint));
                //
        ...
        private string MakeMteRelayHeaderValue(MteRelayEndpoint endpoint, bool isGetRequest = false)
        {
            //
            // Set the options that should be in the x-mte-relay header.
            //
            endpoint.XmteRelayHeader.bodyIsEncoded = _mteRelayOptions.ShouldEncodeBody;
            if (isGetRequest) endpoint.XmteRelayHeader.bodyIsEncoded = false; // Do not encode a body for the GET requests.
            endpoint.XmteRelayHeader.urlIsEncoded = _mteRelayOptions.ShouldEncodeUrl;
            switch (_mteRelayOptions.HeaderDisposition)
            {
            ...
            try
            {
                ...
                foreach (var pairId in endpoint.PairIds!)
                {
                    ...
                    if (badEncoderState != null)
                    {
                        ...
                        //await _mteHelper.RemoveFromSDRAsync($"E-{x_mte_relay_id}");
                    }
                    ...
                    if (badDecoderState != null)
                    {
                        string key = $"D-{pairId}";
                        _mteStateDictionary.Remove<string, string>(key, out _);
                        //await _mteHelper.RemoveFromSDRAsync($"D-{x_mte_relay_id}");
                    }
                }
        ...
        private async Task<(Dictionary<string, string> clearHeaders, string encodedHeaders)> MakeEncodedPayloadHeader(MteRelayEndpoint endpoint, HttpMethod method, Dictionary<string, string>? headers, string defaultContentType)
        {
            try
            {
                ...
                //
                if (headers is null || headers.Count == 0 || !headers.ContainsKey(Constants.MTE_RELAY_HEADER))
                {
                    throw new ApplicationException($"You must include a {Constants.MTE_RELAY_HEADER} header in your headers collection.");
                }
                //
                ...
                //
                string x_mte_relay_id = headers[Constants.MTE_RELAY_HEADER];
                clearHeaders.Add(Constants.MTE_RELAY_HEADER, x_mte_relay_id);
                //
                ...
                if (headersToEncode.Count > 0)
                {
                    ...
                    {
                        throw new ApplicationException($"No MteEncoder is found for x_mte_relay_id: {x_mte_relay_id}");
                    }
                    //
...
#pragma warning restore CS8600 // Converting null literal or possible null value to non-nullable type.
        ...
        /// <summary>
        /// Helper (static) to log out a byte array.
        /// </summary>
