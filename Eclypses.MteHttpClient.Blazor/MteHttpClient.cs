...
namespace MteHttpClient
{
    public class MteHttpClient : IMteHttpClient
    {
        ...
        /// <summary>
        /// A factory method for creating ECDH objects used in pairing "pairId" designated Mte objects.
        /// </summary>
        private readonly IEcdhFactory _ecdhFactory;
        /// <summary>
        ...
        public async Task<bool> InitializeAsync(MteRelayEndpoint endpoint, bool shouldReset = false)
        {
            try
            {
                ...
                if (!endpoint.IsPaired)
                {
                    ...
                    //
                    CreateMteRelayClient(endpoint.RelayIdentifier);
                    //
            ...
            catch (Exception ex)
            {
                ...
                throw new Exception($"Could not setup the Mte-Relay proxy for relayId {endpoint.RelayIdentifier}.", ex);
            }
        }
        ...
        /// <exception cref="ApplicationException"></exception>
        /// <exception cref="Exception"></exception>
        public async Task<HttpResponseMessage> MteGetAsync(string route, Dictionary<string, string>? headers = null, string? relayIdentifier = "", int getAttempts = 0)
        {
            ...
            try
            {
                ...
                headers.Remove(Constants.RELAY_PAIR_ID_HEADER);
                string pairId = GetNextPairId(endpoint);
                headers.Add(Constants.RELAY_PAIR_ID_HEADER, pairId);
                //
                // Create the encoded header collection (includes the content-type header).
                //
                (Dictionary<string, string> clearHeaders, string encodedHeaders) = await MakeEncodedPayloadHeader(endpoint, HttpMethod.Get, headers, "text/plain");
                ...
                //
                await CreateDecodedHttpResponseMessage(pairId, relayResponse);
            ...
            catch (MteRelayException mrex)
            {
                ...
                {
                    var retryResponse = await MteGetAsync(route, headers, relayIdentifier, getAttempts);
                    //
...
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
        ...
        public async Task<string> MteGetStringAsync(string route, Dictionary<string, string>? headers = null, string? relayIdentifier = "", int getAttempts = 0)
        {
            HttpResponseMessage? responseMessage = null;
            try
            {
                responseMessage = await MteGetAsync(route, headers, relayIdentifier, getAttempts);
        ...
        public async Task<byte[]> MteGetByteArrayAsync(string route, Dictionary<string, string>? headers = null, string? relayIdentifier = "", int getAttempts = 0)
        {
            HttpResponseMessage? responseMessage = null;
            try
            {
...
#pragma warning restore CS1998 // Async method lacks 'await' operators and will run synchronously
        ...
        public async Task<HttpResponseMessage> MtePostAsync(string route, HttpContent content, Dictionary<string, string>? headers = null, string? relayIdentifier = "", int postAttempts = 0)
        {
            MteRelayEndpoint endpoint = CreateMteRelayClient(relayIdentifier);
            HttpResponseMessage? relayResponse = null;
            ...
            try
            {
                ...
                {
                    (clearHeaders, encodedHeaders) = await MakeEncodedPayloadHeader(endpoint, HttpMethod.Post, headers, "application/json");
                    content = await MakeEncodedContent(await content.ReadAsStringAsync(), pairId);
                }
                else if (content.GetType() == typeof(StringContent))
                {
                    (clearHeaders, encodedHeaders) = await MakeEncodedPayloadHeader(endpoint, HttpMethod.Post, headers, "text/plain");
                    content = await MakeEncodedContent(await content.ReadAsStringAsync(), pairId);
                }
                else if (content.GetType() == typeof(ByteArrayContent))
                {
                    (clearHeaders, encodedHeaders) = await MakeEncodedPayloadHeader(endpoint, HttpMethod.Post, headers, "application/octet-stream");
                    content = await MakeEncodedContent(await content.ReadAsByteArrayAsync(), pairId);
                }
                //
                // Make a request to send to the relay server.
                //
                ...
                // Convert the relayResponse to a clearResponse for the application to consume.
                //
                await CreateDecodedHttpResponseMessage(pairId, relayResponse, originalContent);
                return relayResponse;
            ...
            catch (MteRelayException mrex)
            {
                if (endpoint.RetryAttempts <= postAttempts)
                {
                    ...
                    //
                    var retryResponse = await MtePostAsync(route, mrex.OriginalContent!, headers, relayIdentifier, postAttempts);
                    //
                    // This will remove the failed mte states and request
                    // a new pair id from the MTERelay endpointInformation server. It is an
                    // async method, but we do not wish to await it so that
...
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
        ...
        /// <returns>HttpResponseMessage of what was received from the API (after being protected with Mte Relay).</returns>
        public async Task<HttpResponseMessage> MtePatchAsync(string route, HttpContent content, Dictionary<string, string>? headers = null, string? relayIdentifier = "", int patchAttempts = 0)
        {
            MteRelayEndpoint endpoint = CreateMteRelayClient(relayIdentifier);
            HttpResponseMessage? relayResponse = null;
            try
            {
                ...
                if (content.GetType() == typeof(JsonContent))
                {
                    (clearHeaders, encodedHeaders) = await MakeEncodedPayloadHeader(endpoint, HttpMethod.Patch, headers, "application/json");
                    content = await MakeEncodedContent(await content.ReadAsStringAsync(), pairId);
                }
                else if (content.GetType() == typeof(StringContent))
                {
                    (clearHeaders, encodedHeaders) = await MakeEncodedPayloadHeader(endpoint, HttpMethod.Patch, headers, "text/plain");
                    content = await MakeEncodedContent(await content.ReadAsStringAsync(), pairId);
                }
                else if (content.GetType() == typeof(ByteArrayContent))
                {
                    (clearHeaders, encodedHeaders) = await MakeEncodedPayloadHeader(endpoint, HttpMethod.Patch, headers, "application/octet-stream");
                    content = await MakeEncodedContent(await content.ReadAsByteArrayAsync(), pairId);
                }
                //
                // Make a request to send to the relay server.
            ...
            catch (MteRelayException mrex)
            {
                if (endpoint.RetryAttempts <= patchAttempts)
                {
                    ...
                    // Retry this method with the original content which is a property of the MteRelayException.
                    //
                    var retryResponse = await MtePatchAsync(route, mrex.OriginalContent!, headers, relayIdentifier, patchAttempts);
                    //
                    // This will remove the failed mte states and request
                    // a new pair id from the MTERelay endpointInformation server. It is an
                    // async method, but we do not wish to await it so that
...
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
        ...
        /// <returns>HttpResponseMessage of what was received from the API (after being protected with Mte Relay).</returns>
        public async Task<HttpResponseMessage> MtePutAsync(string route, HttpContent content, Dictionary<string, string>? headers = null, string? relayIdentifier = "", int putAttempts = 0)
        {
            MteRelayEndpoint endpoint = CreateMteRelayClient(relayIdentifier);
            HttpResponseMessage? relayResponse = null;
            try
            {
                ...
                if (content.GetType() == typeof(JsonContent))
                {
                    (clearHeaders, encodedHeaders) = await MakeEncodedPayloadHeader(endpoint, HttpMethod.Put, headers, "application/json");
                    content = await MakeEncodedContent(await content.ReadAsStringAsync(), pairId);
                }
                else if (content.GetType() == typeof(StringContent))
                {
                    (clearHeaders, encodedHeaders) = await MakeEncodedPayloadHeader(endpoint, HttpMethod.Put, headers, "text/plain");
                    content = await MakeEncodedContent(await content.ReadAsStringAsync(), pairId);
                }
                else if (content.GetType() == typeof(ByteArrayContent))
                {
                    (clearHeaders, encodedHeaders) = await MakeEncodedPayloadHeader(endpoint, HttpMethod.Put, headers, "application/octet-stream");
                    content = await MakeEncodedContent(await content.ReadAsByteArrayAsync(), pairId);
                }
                //
                // Make a request to send to the relay server.
            ...
            catch (MteRelayException mrex)
            {
                if (endpoint.RetryAttempts <= putAttempts)
                {
                    ...
                    // Retry this method with the original content which is a property of the MteRelayException.
                    //
                    var retryResponse = await MtePutAsync(route, mrex.OriginalContent!, headers, relayIdentifier, putAttempts);
                    //
                    // This will remove the failed mte states and request
                    // a new pair id from the MTERelay endpointInformation server. It is an
                    // async method, but we do not wish to await it so that
...
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
        ...
        private MteRelayEndpoint? GetEndpointFromId(string relayIdentifier)
        {
            ...
            return _mteRelayOptions.Endpoints.Where(e => e.RelayIdentifier!.Equals(relayIdentifier, StringComparison.OrdinalIgnoreCase)).FirstOrDefault();
        }
        #endregion
        ...
        private MteRelayEndpoint CreateMteRelayClient(string? relayIdentifier)
        {
            ...
            {
                endpoint.mteRelayClient = _httpClientFactory.CreateClient(relayIdentifier!);
                endpoint.mteRelayClient.BaseAddress = new Uri(endpoint.MteRelayUrl!);
            }
            //
            // This should allow cookies to propagate from the browser..... (TODO: Test this)
            //
            SetDefaultRequestHeader(relayIdentifier!, "credentials", "include");
            return endpoint;
        }
        #endregion
        ...
        /// </summary>
        /// <param name="endpoint">The specifi endpoint you are creating the MteState pairs for.</param>
        ...
        private async Task<bool> VerifyMteRelay(MteRelayEndpoint endpoint)
        {
            ...
            if (!string.IsNullOrWhiteSpace(endpoint.ApiEchoRoute))
            {
                ...
                // 
                string echoString = await MteGetStringAsync(endpoint.ApiEchoRoute, relayIdentifier: endpoint.RelayIdentifier);
                //
                ...
                {
                    throw new ApplicationException($"{endpoint.ApiEchoRoute} did not complete - MteRelay or API is not available for {endpoint.RelayIdentifier}.");
                }
                //
                ...
                else
                {
                    ...
                    {
                        throw new ApplicationException($"The route at {endpoint.ApiEchoRoute} for {endpoint.RelayIdentifier} should have returned a string containing {endpoint.ApiEchoString}, but it did not.");
                    }
                }
            }
            else
        ...
        /// <summary>
        /// Gets the next available pairId in a round-robin fashion
        /// for use in determining which Encoder / Decoder pair that
        /// should be used.
        /// </summary>
        /// <returns>The next pairId from the list of pairIds.</returns>
        ...
        private async Task<bool> CheckForMTERelayProxy(MteRelayEndpoint endpoint)
        {
            try
            {
                ...
                {
                    throw new ApplicationException($"You must supply a base address for the Mte-Relay endpoint at {endpoint.RelayIdentifier}.");
                }
                //
                ...
                //
                // Save the special headers from this initial relayResponse.
                //
                endpoint.MteRelaySpecialHeaders.Clear();
#pragma warning disable CS8604 // Possible null reference argument.
                foreach (var header in httpResponseMsg.Headers)
                {
                    if (header.Key == Constants.RELAY_SERVER_ID_HEADER && header.Value != null)
                    {
                        endpoint.MteRelaySpecialHeaders.TryAdd(Constants.RELAY_SERVER_ID_HEADER, header.Value.FirstOrDefault());
                    }
                    else if (header.Key == Constants.RELAY_CLIENT_ID_HEADER && header.Value != null)
                    {
                        endpoint.MteRelaySpecialHeaders.TryAdd(Constants.RELAY_CLIENT_ID_HEADER, header.Value.FirstOrDefault());
                    }
                }
#pragma warning restore CS8604 // Possible null reference argument.
                //
                // You should get at least 2 headers back from the Head request.
                //
                if (endpoint.MteRelaySpecialHeaders.Count < 2)
                {
                    throw new ApplicationException($"The server at {endpoint.MteRelayUrl} is not an Mte-Relay server - the serverId and clientId were not returned from the Mte-Relay.");
                }
                return true;
            }
            catch (Exception ex)
        ...
        private async Task<bool> PairWithTheMTERelay(MteRelayEndpoint endpoint)
        {
            try
            {
                ...
                //
                // Prepare the lists of MtePairRequestModels and ECDH objects that are needed for creating the multiple pairs.
                //
                List<MtePairRequestModel> pairValues = new List<MtePairRequestModel>(pairIdsToCreate);
                Dictionary<string, IEcdhHelperMethods> encoderECDHList = new Dictionary<string, IEcdhHelperMethods>(pairIdsToCreate);
                Dictionary<string, IEcdhHelperMethods> decoderECDHList = new Dictionary<string, IEcdhHelperMethods>(pairIdsToCreate);
                //
                // Create ECDH objects for each item in the list
                ...
                for (int i = 0; i < pairIdsToCreate; i++)
                {
                    ...
                    encoderECDHList[pairId] = _ecdhFactory!.Create();
                    decoderECDHList[pairId] = _ecdhFactory!.Create();
                    pairValues.Add(await MakeSinglePairRequestModel(pairId, encoderECDHList, decoderECDHList));
                }
                //
                // Ensure that the required headers are present.
                //
                EnsureRequestHeader(endpoint, Constants.RELAY_CLIENT_ID_HEADER);
                //
                ...
                //
                CheckResponseHeader(Constants.RELAY_SERVER_ID_HEADER, responseMsg);
                CheckResponseHeader(Constants.RELAY_CLIENT_ID_HEADER, responseMsg);
                //
                ...
                //
                List<MtePairRequestModel>? serverPairRequestList = JsonSerializer.Deserialize<List<MtePairRequestModel>>(json, _jsonOptions);
                ...
                //
                // Create an SDR instance to securely store State Objects.
                //
                await CreateTheSDRForLocalStateStorage();
                //
                // Use the ECDH modules to create the Entropy needed for all of the MTE Pairs for this endpoint.
                //
                await CreateTheEntropyKeyPairs(pairValues, encoderECDHList, decoderECDHList, serverPairRequestList);
                return true;
            }
            catch (Exception ex)
            {
        ...
        #region private CreateTheSDRForLocalStateStorage
        /// <summary>
        /// Creates an SDR as secure Session Storage
        /// for the MTE States.
        /// </summary>
        /// <returns></returns>
        /// <exception cref="Exception"></exception>
        private async Task CreateTheSDRForLocalStateStorage()
        {
            try
            {
                await _mteHelper.InitializeSDRAsync();
            }
            catch (Exception ex)
            {
                throw new Exception("Could not create a Secure Data Repository for storing MteState.", ex);
            }
        }
        #endregion CreateTheSDRForLocalStateStorage

        #region private EnsureRequestHeader
        /// <summary>
        /// Checks to be sure a specific headerName is present, and if not, it adds it.
        /// </summary>
        ...
        private void EnsureRequestHeader(MteRelayEndpoint endpoint, string headerKey)
        {
            if (!endpoint!.mteRelayClient!.DefaultRequestHeaders.Contains(headerKey))
            {
                endpoint.mteRelayClient.DefaultRequestHeaders.Add(headerKey, endpoint.MteRelaySpecialHeaders[headerKey]);
            }
        }
        ...
        private async Task<MtePairRequestModel> MakeSinglePairRequestModel(string pairId, Dictionary<string, IEcdhHelperMethods> encoderECDHList, Dictionary<string, IEcdhHelperMethods> decoderECDHList)
        {
            try
            {
                //
                // Instance up a couple of ECDH objects and create the key pairs.
                //
                IEcdhHelperMethods? encoderECDH = encoderECDHList[pairId];
                IEcdhHelperMethods? decoderECDH = decoderECDHList[pairId];
                //
                // Create the key pairs for the Encoder and Decoder
                // using Elliptical Curve Diffie-Hellman. Note, the
                // keys to exchange with the Mte Relay Proxy server
                // must contain an extra byte due to the way JavaScript
                // works for ECDH.
                //
                byte[] encoderPublicKey = new byte[encoderECDH.SzPublicKey + 1];
                byte[] decoderPublicKey = new byte[decoderECDH.SzPublicKey + 1];
                int rc = 0;

                (rc, encoderPublicKey) = await encoderECDH.GetPublicKeyFromECDHAsync();
                if (rc != 0)
                {
                    throw new ApplicationException($"Could not create an ECDH public key for the encoder. rc is {rc}");
                }

                (rc, decoderPublicKey) = await decoderECDH.GetPublicKeyFromECDHAsync();
                if (rc != 0)
                {
                    throw new ApplicationException($"Could not create an ECDH public key for the decoder. rc is {rc}");
                }

                encoderPublicKey[0] = 4;
                decoderPublicKey[0] = 4;
                //
                ...
                MtePairRequestModel clientPairRequest = new MtePairRequestModel
                {
                    ...
                    decoderPublicKey = Convert.ToBase64String(decoderPublicKey),
                    encoderPersonalizationStr = encoderPersonalizationString,
                    encoderPublicKey = Convert.ToBase64String(encoderPublicKey),
                };
        ...
        /// <param name="encoderECDHList">The list of the encoder ECDH objects.</param>
        /// <param name="decoderECDHList">The list of the decoder ECDH objects.</param>
        /// <param name="serverPairRequestList">The pairing values from the MteRelay server.</param>
        /// <returns>Complete Task</returns>
        /// <exception cref="ApplicationException"></exception>
        /// <exception cref="Exception"></exception>
        private async Task CreateTheEntropyKeyPairs(List<MtePairRequestModel> pairValues, Dictionary<string, IEcdhHelperMethods> encoderECDHList, Dictionary<string, IEcdhHelperMethods> decoderECDHList, List<MtePairRequestModel>? serverPairRequestList)
        {
            try
            {
                //
                ...
                //
                int rc = 0;
                byte[]? tempEncoderPublicKey = null;
                byte[]? tempDecoderPublicKey = null;
                foreach (var clientPairRequest in pairValues)
                {
                    //
                    // Get an encoder and decoder from the list that was created for the specified pairId
                    // and allocate some byte arrays if they are null.
                    //
                    IEcdhHelperMethods? encoderECDH = encoderECDHList[clientPairRequest.pairId!];
                    IEcdhHelperMethods? decoderECDH = decoderECDHList[clientPairRequest.pairId!];
                    tempEncoderPublicKey ??= new byte[encoderECDH.SzPublicKey + 1];
                    tempDecoderPublicKey ??= new byte[decoderECDH.SzPublicKey + 1];
                    byte[]? decoderEntropy = null;
                    byte[]? encoderEntropy = null;
                    ...
                    //
                    // Get the entropy from the ECDH object for the encoder and decoder.
                    // Note: the returned public key from Javascript has an extra byte
                    // prepended to it, so strip that away.
                    //
                    tempEncoderPublicKey = Convert.FromBase64String(serverPairRequest.encoderPublicKey!);
                    tempDecoderPublicKey = Convert.FromBase64String(serverPairRequest.decoderPublicKey!);
                    byte[]? serverEncoderPublicKey = null;
                    byte[]? serverDecoderPublicKey = null;

                    serverEncoderPublicKey = new byte[encoderECDH.SzPublicKey + 1];
                    Array.Copy(tempEncoderPublicKey, 0, serverEncoderPublicKey, 0, encoderECDH.SzPublicKey + 1);
                    serverDecoderPublicKey = new byte[encoderECDH.SzPublicKey + 1];
                    Array.Copy(tempDecoderPublicKey, 0, serverDecoderPublicKey, 0, decoderECDH.SzPublicKey + 1);

                    (rc, decoderEntropy) = await decoderECDH.ComputeSharedSecretAsync(serverEncoderPublicKey);
                    if (rc != 0)
                    {
                        throw new ApplicationException($"Could not get a shared secret for the decoder with pairId {clientPairRequest.pairId} rc is: {rc}");
                    }

                    (rc, encoderEntropy) = await encoderECDH.ComputeSharedSecretAsync(serverDecoderPublicKey);
                    if (rc != 0)
                    {
                        throw new ApplicationException($"Could not get a shared secret for the encoder with pairId {clientPairRequest.pairId} rc is: {rc}");
                    }

                    await MakeSingleClientMKEPair(clientPairRequest, serverPairRequest, decoderEntropy, encoderEntropy);
                    ...
                    Array.Clear(tempDecoderPublicKey);
                    Array.Clear(tempEncoderPublicKey);
                }
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Creates the actual instances of the MKE Encoder and Decoder for this client'specialHeader pairId.
        /// </summary>
        ...
        /// <exception cref="ApplicationException"></exception>
        private async Task MakeSingleClientMKEPair(MtePairRequestModel clientPairRequest, MtePairRequestModel? serverPairRequest, byte[] decoderEntropy, byte[] encoderEntropy)
        {
            try
            {
                //
                ...
                await PutEncoderStateintoSDR(clientPairRequest.pairId!, theEncoderState);
                ...
                await PutDecoderStateintoSDR(clientPairRequest.pairId!, theDecoderState);
            }
            catch (Exception ex)
            {
        ...
        /// <param name="pairId">The specific pairId to replace.</param>        
        private async Task ReplacePairedEndpoint(MteRelayEndpoint endpoint)
        {
            ...
            {             
                ...
                {
                    var badEncoderState = await GetEncoderStateFromSDR(pairId);

                    if (badEncoderState != null)
                    {
                        await _mteHelper.RemoveFromSDRAsync($"E-{pairId}");
                    }

                    var badDecoderState = await GetDecoderStateFromSDR(pairId);
                    if (badDecoderState != null)
                    {
                        await _mteHelper.RemoveFromSDRAsync($"D-{pairId}");
                    }
                }
        ...
        private HttpRequestMessage MakeHttpRelayRequest(HttpContent? content, string route, HttpMethod method, string encodedHeaders, Dictionary<string, string> clearHeaders)
        {
            try
            {
                ...
                {
                    request.Headers.Add(Constants.RELAY_ENCODED_HEADERS_HEADER, encodedHeaders);
                }
                return request;
            }
            catch (Exception ex)
        ...
        private async Task<(Dictionary<string, string> clearHeaders, string encodedHeaders)> MakeEncodedPayloadHeader(MteRelayEndpoint endpoint, HttpMethod method, Dictionary<string, string>? headers, string defaultContentType)
        {
            try
            {
                //
                // Ensure that the PairId header is present.
                //
                if (headers is null || headers.Count == 0 || !headers.ContainsKey(Constants.RELAY_PAIR_ID_HEADER))
                {
                    throw new ApplicationException($"You must include a {Constants.RELAY_PAIR_ID_HEADER} header in your headers collection.");
                }
                //
                // Add of replace any DefaultRequestHeaders for this endpoint to the headers collection.
                //
                ...
                //
                // Add the PairId header to the un-encoded headers.
                //
                string pairId = headers[Constants.RELAY_PAIR_ID_HEADER];
                clearHeaders.Add(Constants.RELAY_PAIR_ID_HEADER, pairId);
                //
                // Add the other "special" headers used for the Mte-Relay.
                //
                foreach (var specialHeader in endpoint.MteRelaySpecialHeaders)
                {
                    clearHeaders.Add(specialHeader.Key, specialHeader.Value);
                }
                //
                ...
                //
                if (_mteRelayOptions!.HeaderDisposition == RelayHeaderDisposition.EncodeAllHeaders)
                {
                ...
                //
                else if (_mteRelayOptions.HeadersToEncode is not null && _mteRelayOptions.HeaderDisposition == RelayHeaderDisposition.EncodeListOfHeaders)
                {
                    foreach (string key in headers.Keys)
                    {
                        if (!key.Equals(Constants.STR_CONTENT_TYPE_HEADER, StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            {
                                if (_mteRelayOptions.HeadersToEncode.Contains(key))
                                {
                ...
                //
                else if (_mteRelayOptions.HeaderDisposition == RelayHeaderDisposition.EncodeNoHeaders)
                {
                    foreach (string key in headers.Keys)
                    {
                        if (!key.Equals(Constants.STR_CONTENT_TYPE_HEADER, StringComparison.OrdinalIgnoreCase))
                ...
                //
                // If we have some to encode (we should always have content-type), then encode them
                // to include in the encoded headers header.
                //
                string encodedHeaders = string.Empty;
                if (headersToEncode.Count > 0)
                {
                    ...
                    //
                    await GetCurrentEncoderStateAndStoreInSDR(pairId, mkeEncoder);
                    _mteHelper.PutTheEncoderAsync(mkeEncoder);
        ...
        /// </summary>
        /// <param name="response">The encoded relayResponse from the MteRelay endpointInformation server.</param>
        ...
        private async Task<HttpResponseMessage> MakeClearResponseMessage(HttpResponseMessage response)
        {
            ...
            try
            {
                ...
                KeyValuePair<string, IEnumerable<string>>? encodedMteRelayHeaders = response.Headers
                    .Where(h => h.Key == Constants.RELAY_ENCODED_HEADERS_HEADER)
                    .FirstOrDefault();
                //
                ...
                {
                    throw new ApplicationException($"Mte-Relay must return a {Constants.RELAY_ENCODED_HEADERS_HEADER} header and none was returned.");
                }
                if (!encodedMteRelayHeaders.HasValue)
                {
                    throw new ApplicationException($"Mte-Relay return a {Constants.RELAY_ENCODED_HEADERS_HEADER} header but it was empty.");
                }
                //
                // The relayResponse must include a header value for this pair id
                //
                var pairIdHeader = response.Headers.Where(h => h.Key == Constants.RELAY_PAIR_ID_HEADER).FirstOrDefault();
                if (pairIdHeader.Value is null)
                {
                    throw new ApplicationException($"The relayResponse requires a {Constants.RELAY_PAIR_ID_HEADER} header and none was returned.");
                }
                //
                // Find out which pair we are working with.
                //
                string pairId = pairIdHeader.Value.First();
                //
                ...
                if (string.IsNullOrWhiteSpace(encodedRelayHeaders))
                {
                    var endpoint = GetEndpointFromId(pairId);
                    throw new ApplicationException($"Http relayResponse from Mte-Relay at {endpoint!.MteRelayUrl} did not return an encoded Header for pairId {pairId}.");
                }
                ...
                if (string.IsNullOrWhiteSpace(contentTypeKey))
                {
                    throw new ApplicationException($"Mte-Relay must return a 'content-type' header in its {Constants.RELAY_ENCODED_HEADERS_HEADER} header collection - it did not.");
                }
                string contentType = decodedRelayHeaders[contentTypeKey];
                if (string.IsNullOrEmpty(contentType))
                {
                    throw new ApplicationException($"Mte-Relay must return a valid 'content-type' header in its {Constants.RELAY_ENCODED_HEADERS_HEADER} header collection, but it was blank.");
                }
                ...
                if (response.Content is not null && response.StatusCode != HttpStatusCode.NoContent)
                {
                    ...
                    foreach (var h in response.Content.Headers)
                    {
                        ...
                        //
                        if (h.Key.ToLower() == "content-type" || h.Key.ToLower() == "content-length")
                        {
                            continue;
                        }
                ...
                //
                if (response.Headers.Contains(Constants.RELAY_ENCODED_HEADERS_HEADER))
                {
                    response.Headers.Remove(Constants.RELAY_ENCODED_HEADERS_HEADER);
                }
                return response;
            }
            catch (Exception ex)
        ...
        /// in a header with the key of Constants.RELAY_ENCODED_HEADERS_HEADER
        ...
        /// <param name="pairId">This is the pairId to decode with.</param>
        ...
        private async Task<Dictionary<string, string>?> MteDecodeCustomHeaderCollection(string encoded, string pairId)
        {
            try
            {
                ...
                {
                    throw new ApplicationException($"No Decoder found for pairId {pairId}");
                }
                //
                // Decode the encoded header into a Dictionary.
                //
                string clearHeaders = await _mteHelper.DecodeToStringAsync(mkeDecoder, encoded);
                await GetCurrentDecoderStateAndStoreInSDR(pairId, mkeDecoder);
                _mteHelper.PutTheDecoderAsync(mkeDecoder);

                if (string.IsNullOrWhiteSpace(clearHeaders))
                {
                    throw new ApplicationException($"Could not MTE decode the {Constants.RELAY_ENCODED_HEADERS_HEADER} headerName.");
                }
        ...
        /// <param name="pairId">The specific pairId of the MteState to hydrate with.</param>
        ...
        /// <summary>
        /// Gets the state from the SDR for this encoder and hydrates the encoder with it.
        /// </summary>
        ...
        private async Task HydrateTheEncoder(string pairId, IJSObjectReference mkeEncoder)
        {
            try
            {
                var states = await GetEncoderStateFromSDR(pairId);
                if (states is null)
                {
                    throw new ApplicationException($"No MteEncoder states found for pairId: {pairId}");
                }

                if (string.IsNullOrWhiteSpace(states.B64State))
                {
                    throw new ApplicationException($"MteEncoder states is empty for pairId: {pairId}");
                }
        ...
        /// <summary>
        /// Gets the state from the SDR for this decoder and hydrates the encoder with it.
        /// </summary>
        ...
        private async Task HydrateTheDecoder(string pairId, IJSObjectReference mkeDecoder)
        {
            try
            {
                MteState states = await GetDecoderStateFromSDR(pairId);
                if (string.IsNullOrEmpty(states.B64State))
                {
                    throw new ApplicationException($"MteDecoder states is empty for pairId: {pairId}");
                }
        ...
        #region private GetDecoderStateFromSDR
        /// <summary>
        ///  Retrieves the current state from the SDR for the designated pairId.
        /// </summary>
        /// <param name="pairId">The pairid of the decoder you wish to retrieve.</param>
        /// <returns>The retrieved state.</returns>
        /// <exception cref="ApplicationException"></exception>
        /// <exception cref="Exception"></exception>
        private async Task<MteState> GetDecoderStateFromSDR(string pairId)
        {
            try
            {
                string? statesJson = await _mteHelper.RetrieveFromSDRAsync($"D-{pairId}");
                if (string.IsNullOrWhiteSpace(statesJson))
                {
                    throw new ApplicationException($"No MteState found in SDR for pairId {pairId}.");
                }
...
#pragma warning restore CS8600 // Converting null literal or possible null value to non-nullable type.
                if (states is null)
                {
                    throw new ApplicationException($"No MteDecoder states found for pairId: {pairId}");
                }
                return states;
            }
            catch (Exception ex)
            {
                throw new Exception("Exception getting Decoder State from SDR", ex);
            }
        }
        #endregion

        #region private GetEncoderStateFromSDR
        /// <summary>
        ///  Retrieves the current state from the SDR for the designated pairId.
        /// </summary>
        ...
        /// <exception cref="ApplicationException"></exception>
        /// <exception cref="Exception"></exception>
        private async Task<MteState> GetEncoderStateFromSDR(string pairId)
        {
            try
            {
                string? statesJson = await _mteHelper.RetrieveFromSDRAsync($"E-{pairId}");
                if (string.IsNullOrWhiteSpace(statesJson))
                {
                    throw new ApplicationException($"No MteState found in SDR for pairId {pairId}.");
                }
...
#pragma warning restore CS8600 // Converting null literal or possible null value to non-nullable type.
            ...
            {
                throw new Exception("Exception getting Encoder State from SDR", ex);
            }
        }
        #endregion

        #region private PutDecoderStateintoSDR
        /// <summary>
        /// Places the current state of the MTE into the SDR.
        /// </summary>
        /// <param name="pairid">The pairId associated with the decoder.</param>
        /// <param name="states">The current object with the state.</param>
        /// <returns>Completed Task.</returns>
        /// <exception cref="Exception"></exception>
        private async Task PutDecoderStateintoSDR(string pairid, MteState states)
        {
            try
            {
                await _mteHelper.StoreInSDRAsync($"D-{pairid}", JsonSerializer.Serialize(states));
            }
            catch (Exception ex)
            {
                throw new Exception("Exception putting Decoder State into SDR", ex);
            }
        }
        #endregion

        #region private PutEncoderStateintoSDR
        /// <summary>
        /// Places the current state of the MTE into the SDR.
        /// </summary>
        /// <param name="pairid">The pairId associated with the encoder.</param>
        /// <param name="states">The current object with the state.</param>
        /// <returns>Completed Task.</returns>
        /// <exception cref="Exception"></exception>
        private async Task PutEncoderStateintoSDR(string pairid, MteState states)
        {
            try
            {
                await _mteHelper.StoreInSDRAsync($"E-{pairid}", JsonSerializer.Serialize(states));
            }
            catch (Exception ex)
            {
                throw new Exception("Exception putting Encoder State into SDR", ex);
            }
        }
        #endregion

        #region private GetCurrentEncoderStateAndStoreInSDR
        /// <summary>
        /// Gets the Encoder state from an MTE and stores it in the SDR.
        /// </summary>
        ...
        /// <returns>Completed Task</returns>
        /// <exception cref="ApplicationException"></exception>
        private async Task GetCurrentEncoderStateAndStoreInSDR(string pairId, IJSObjectReference mkeEncoder)
        {
            var states = await GetEncoderStateFromSDR(pairId);
            if (states is null)
            {
            ...
            await PutEncoderStateintoSDR(pairId, states);
        }
        #endregion

        #region private GetCurrentDecoderStateAndStoreInSDR        
        /// <summary>
        /// Gets the Decoder state from an MTE and stores it in the SDR.
        /// </summary>
        /// <param name="pairId">The specific pairid - used as the "key" to storage</param>
        /// <param name="mkeEncoder">The specific Decoder that has the state to save.</param>
        /// <returns>Completed Task</returns>
        /// <exception cref="ApplicationException"></exception>
        private async Task GetCurrentDecoderStateAndStoreInSDR(string pairId, IJSObjectReference mkeDecoder)
        {
            var states = await GetDecoderStateFromSDR(pairId);
            if (states is null)
            {
                throw new ApplicationException($"No MteDecoder states found for pairId: {pairId}");
            }
            states.B64State = await _mteHelper.RetrieveDecoderStateAsync(mkeDecoder);
            await PutDecoderStateintoSDR(pairId, states);
        }
        #endregion
